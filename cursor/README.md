...
本仓库中 skills 位于 `cursor/` 子目录；安装后平铺到客户端 skill home：
...
~/.cursor/skills/             # npx --cursor 或 make install-cursor 安装结果
  planning/
  building/
  ...
```
...
通过 `npx` 从 GitHub 拉取仓库，把 `cursor/` 里的 skill 平铺到本地 skill 目录：
