...
把整个仓库放在工作区根目录的 `.cursor/skills` 下。Cursor 会读取本目录下的 `SKILL.md`，按 description 触发。

### 在 Claude Code 中

把目录复制到 `~/.claude/skills/` 或项目内 `.claude/skills/`。Claude Code 自动加载，按描述触发。

### 在其他 AI 客户端

只要支持 Anthropic Agent Skills 标准（`SKILL.md` + frontmatter `name` + `description`）即可。
