...
- [ ] 设计卡 含 S/B 拆分顺序
- [ ] 失败模式 ≥3 条且每条有验证项
- [ ] 命中落盘触发条件时（设计卡 / large / 方案卡非小型）：MD 文档落盘路径已规划（`docs/astra/<date>-<slug>.md`）
...
**硬门：触发任一条件时，必须把执行卡写成 MD 文件落盘后，才允许 building skill 开始编码。**
...
- 大需求跨多次对话 / 多 agent / 多 commit，对话上下文不可靠
- AI 写代码越界时，需要可引用的契约文档对齐回去（Non-Goals / diff 预算 / 切片边界）
- 决策记录留盘，未来回看不靠 git log 反推

落盘规则：

- **路径**：统一写到 `docs/astra/<YYYY-MM-DD>-<slug>.md`；目录不存在时先建。slug 用 kebab-case，简短描述需求（如 `2026-06-11-user-auth-rewrite.md`）
- **一个需求一份文档**：不拆子目录、不拆"设计 + 评估"两份，所有内容写进同一份文档；同一需求的后续修订直接 edit 原文档，不另起新文件
- **内容**：完整输出骨架（意图 → 决策记录），不得删减字段；档位是设计卡时含决策记录段
- **写入工具**：用 Write 工具直接落盘，不要只在对话里贴 markdown
- **落盘后**：在回复里给出文件绝对路径 + 一句话摘要，再交接 building

不允许：

- "等会再补文档" / "先写代码再补设计" → 顺序倒置直接违规
- "内容已经在对话里了不需要写文件" → 对话会被压缩，必须落盘
- "用户没说要文档" → 命中触发条件即落，不靠用户主动要求
- 同一需求拆多份文档（设计一份 / 评估一份 / 切片一份） → 合并到同一份
...
| "等会再补文档" / "先写代码再补设计" | 命中触发条件必须先落盘 MD，再交 building |
| "内容在对话里了不用写文件" | 用 Write 工具落盘到 `docs/astra/<date>-<slug>.md` |
| "用户没要求要文档所以不写" | 命中触发条件即落，不靠用户主动要求 |
| 同一需求拆"设计文档 + 评估文档"两份 | 合并到同一份 `docs/astra/<date>-<slug>.md` |
...
7. **扫落盘**：命中落盘触发条件时（设计卡 / large / 方案卡非小型），回复里必须含已落盘 `docs/astra/<date>-<slug>.md` 的具体路径（用 Write 工具实际写入，不是只贴 markdown）；缺路径即违规，回 Step 11
