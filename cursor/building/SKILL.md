...
#### 6.0 多 agent 并行评估（编码前必问）

满足**全部**条件时，编码前先用 AskUserQuestion 工具询问用户是否启用多 agent 并行编码：

- 执行卡或任务契约含 ≥2 个相互独立的切片 / 文件 / 模块（不存在强顺序依赖）
...
- Trivial 通道 / 单文件 / 单函数改动
- 单元间存在强依赖（B 必须等 A 落地才能写 → 并行无意义）
- 用户在本次对话已明确表态（"按串行做" / "你直接并行"）
- 设计卡的 S/B 拆分顺序里 S 必须先于 B → S 阶段不并行，B 阶段内独立切片可问

用户选并行后：

- 用 Agent 工具 dispatch 子 agent，每个子 agent 拿到自己的单元契约（Goals / Non-Goals / 涉及文件 / 验证标准）独立执行
- 主 agent 负责：合并 diff、跑统一验证、处理子 agent 越界 / 冲突、按 S/B 顺序提交
- 子 agent 完成回收后，仍要走 Step 7 commit 拆分 + Step 8 验证
...
| "我直接并行帮你做了"（未问就 dispatch 多 agent） | 多单元任务先用 AskUserQuestion 询问并行 / 串行 |
| "全部串行最稳"（多独立单元 + 大 diff 不问就串行） | 满足触发条件即询问，把选择权交用户 |
...
7. **扫并行决策**：满足触发条件（≥2 独立单元 + diff > 100 行或 > 3 文件 + 非 Trivial）时，回复里必须能找到用户对并行/串行的明确选择（AskUserQuestion 结果或显式表态）；缺即违规，回 Step 6.0
