---
name: frontend-product-design-workflow
description: Use when building, modifying, or planning frontend UI, React/Vue/Next/Vite pages, component layouts, dashboards, responsive screens, design systems, frontend architecture, or UI technical choices.
---
...
核心原则：**布局契约优先于视觉装饰。** 复杂 UI 没有布局区域图、槽位职责和溢出规则时，不允许开始写代码。
...
- 纯 diff review：使用 `code-review-workflow`
- 用户明确只要方案：先使用 `solution-design-workflow`，再用本 Skill 做前端约束

## 强制流程

### 1. 识别前端任务类型

先判断当前任务属于哪类，不清楚就先说明判断依据：
...
- 已有相似页面、组件、hook、状态管理和数据请求模式
- lint、typecheck、test、build 命令

禁止没看项目结构就凭通用经验新增一套前端风格。

涉及框架、组件库、状态管理、样式方案或新依赖时，先按 [tech-selection.md](references/tech-selection.md) 做技术选型检查。
...
详细规则见 [layout-modeling.md](references/layout-modeling.md)。
响应式细则见 [responsive-design.md](references/responsive-design.md)。
...
详细模板见 [component-contract.md](references/component-contract.md)。
...
禁止只写“现代、简洁、美观”。必须落到层级、间距、色彩角色和状态表现。

详细规则见 [ui-design-process.md](references/ui-design-process.md)。
...
前端架构规则见 [frontend-architecture.md](references/frontend-architecture.md)。
...
无法验证时必须说明原因和风险，不能写“应该没问题”。

详细检查见 [validation-checklist.md](references/validation-checklist.md) 和 [accessibility-checklist.md](references/accessibility-checklist.md)。
...
- [ ] 是否已经画出界面结构图或说明无需结构图的原因？
- [ ] 是否声明了布局契约：区域、槽位、伸缩、溢出、响应式？
- [ ] 是否定义了复杂组件的 anatomy、slots、states？
- [ ] 是否覆盖 loading、empty、error、disabled、overflow 中相关状态？
- [ ] 是否遵守项目现有前端技术栈和目录结构？
- [ ] 是否完成最小有效验证或明确未验证原因？
